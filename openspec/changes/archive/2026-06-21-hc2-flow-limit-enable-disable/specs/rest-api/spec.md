...
The `PATCH` SHALL accept a JSON body containing an optional `flow_setpoint` value (°C) and/or an optional `active` (boolean) flag. At least one of the two SHALL be present; a `PATCH` with neither SHALL return HTTP 422 without a device write. Behaviour:

- When `flow_setpoint` is present, the service SHALL write `minFl = flow_setpoint` and SHALL ensure `maxFl > minFl` (raising `maxFl` to the device default cap of 65 °C when the current `maxFl` is not strictly greater than the requested `flow_setpoint`).
- The limitation's enabled state SHALL be set to `active` when that flag is present; otherwise, when only `flow_setpoint` is present, the limitation SHALL be enabled (`active = 1`) so a single `flow_setpoint` write both sets the floor and enables it.
- `{"active": false}` SHALL disable the limitation without changing the floor; `{"active": true}` SHALL enable it with the current floor.
...
#### Scenario: Disable the limitation
- **WHEN** `PATCH /api/v1/circuits/hc2/flow-limit` is called with `{"active": false}`
- **THEN** the service sets `active = 0` without changing `minFl`/`maxFl` and returns HTTP 200 with `active` false

#### Scenario: Enable the limitation without changing the floor
- **WHEN** `PATCH /api/v1/circuits/hc2/flow-limit` is called with `{"active": true}`
- **THEN** the service sets `active = 1` using the current `minFl`/`maxFl` and returns HTTP 200 with `active` true

#### Scenario: Empty patch is rejected
- **WHEN** `PATCH /api/v1/circuits/hc2/flow-limit` is called with neither `flow_setpoint` nor `active`
