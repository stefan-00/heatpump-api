...
`docs/ha-integration.md` SHALL provide `configuration.yaml` snippets using HA's `number` REST platform to expose all six HC1 setpoints (`roomOT1`, `roomOT2`, `roomOT3`, `roomOT4`, `roomNO`, `roomSNOT`) as writable number entities. Each entity SHALL read from `GET /api/v1/circuits/hc1/setpoints` and write via `PATCH /api/v1/circuits/hc1/setpoints` with the appropriate JSON body.
...
`docs/ha-integration.md` SHALL provide `configuration.yaml` snippets using HA's `number` REST platform to expose all six HC2 setpoints (`roomOT1`, `roomOT2`, `roomOT3`, `roomOT4`, `roomNO`, `roomSNOT`) as writable number entities. Each entity SHALL read from `GET /api/v1/circuits/hc2/setpoints` and write via `PATCH /api/v1/circuits/hc2/setpoints`.
...
`docs/ha-integration.md` SHALL document that `http://localhost:8765` is the correct base URL when the API runs as an HAOS add-on on the same host, and SHALL recommend a minimum `scan_interval` of 30 seconds to avoid saturating the single HPM device session.
...
- **THEN** the guide instructs them to substitute `localhost` with the host's IP address or hostname
